...
pets %>%
  ## group by primary breed (`primary_breed`)
  group_by(primary_breed) %>% 
  ## count number within each group
  summarise(n=n()) %>%
  ## arrange from most frequent to least frequent
  arrange(-n) %>%
  ## limit to top 3 most frequent breeds
  top_n(n = 3) %>%
...
## and answer the questions in the exercise
dim(steak)
steak %>% filter(Do.you.eat.steak. == "Yes") %>% summarise(n=n())
...
ggsave(plot = p, filename = "steak_plot.png", width = 8, height = 4)
```

## Visualize the Data

```{r mael-meat-plot, warning=FALSE} 
### Explore the data -- create your own plot!
## wrangle the steak data set
male_steak <- filter(steak, Gender == "Male")
male_pref <- male_steak %>% 
  mutate(steak_pref =
         factor(How.do.you.like.your.steak.prepared.,
                             levels = c("Well",
                                        "Medium Well",
                                        "Medium",
                                        "Medium rare",
...
```{r male-visualize, warning=FALSE}
## generate the plot 
p <- ggplot(male_pref) +
  ## specify you want to generate a bar chart
  geom_bar(aes(x = steak_pref, y = prop, fill = steak_pref),
  ...
  scale_fill_manual(values = c("#370B01", 
                           ...
                           "#B5413F")) +
  ## change the scale/labels of the steak-wellness variable (x-axis)
  scale_x_discrete(limits = levels(fct_rev(male_pref$steak_pref)),
                labels = c("Well", 
                           "Medium Well" = "Medium-well",
                           "Medium",
                           "Medium rare" = "Medium-rare",
  ...
  scale_y_continuous(labels = scales::percent, 
                expand = c(mult = c(0,0), 
                           add = c(0,0.035))) +
  ## change the title, subtitle, and caption
  labs(title="'How Do Males Like Their Steak Prepared?'", 
                subtitle="From a survey of 246 steak-eating American Males",
                caption="FiveThirtyEight: Surveymonkey") +
  ## change the theme (use ggthemes)
  theme_bw() +
  ## fine tune the theme
...
```{r save-male-plot, warning=FALSE}
## save plot to figures/explanatory_figures directory
ggsave(plot = p, filename = "male_steak_plot.png", width = 8, height = 4)
```
